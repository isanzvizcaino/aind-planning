...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            #Instanciate a node for every action possible
            a_node = PgNode_a(action)
            #Check if the preconditions of these actions are satisfied in its
            #correspondent level of literals
            issatisfied = True
            ...
            #Include action in the actions that will be added to the level
                a_nodes.append(a_node)
        #Add the level of actions
    ...
    def add_literal_level(self, level):
        ...
        #The variable is instanciated as a set to avoid duplicity
        s_nodes = set()

        #For every action node in the previous A level
        for a_node in self.a_levels[level - 1]:
            #Get all the effects resulting that will be part of the S level
            for effnode in a_node.effnodes:
                #Include it in the set of S nodes to add to the levels
                s_nodes.add(effnode)
                #Connect the nodes to the previous A level
                effnode.parents.add(a_node)
                a_node.children.add(effnode)
        #Add a level of literals
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a):
        ...
        #This test checks if one action negates an effect of the other

        #Check if node_a2 negates any effect from node_a1
        for effect in node_a1.action.effect_add:
            if effect in node_a2.action.effect_rem:
                return True

        #Check if node_a1 negates any effect from node_a2
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a):
        ...
        #This test checks if one of the effects of one action is a negation of
        #a precondition of the other

        #Check if node_a1 effects are opposite to any precondition from node_a2
        for effect in node_a1.action.effect_add:
        ...
        #Check if node_a2 effects are opposite to any precondition from node_a1
        for effect in node_a2.action.effect_add:
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a):
        ...
        #This test checks if one of the preconditions of one action is mutually
        #exclusive with a precondition of the other. Mutual exclusivity relation
        #between preconditions can be checked via is_mutex method of the class
        #PgNode. PgNode_s objects stored as parents of an action are also
        #preconditions. If there is at least one pair of mutually exclusive
        #preconditions, the actions would also be mutually exclusive.
    ...
    def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        #Checks if one literal is a negation of the other. That would mean they
        #have the same expression (PgNode_s.symbol) , but one positive and one negative.
        isnegation = node_s1.symbol == node_s2.symbol and node_s1.is_pos != node_s2.is_pos
        return isnegation
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        #Checks if each possible par of actions that could achieve the two
        #literals is mutually exclusive. The implementation is similar to
        #competing_needs_mutex but, in this case, is_mutex evaluates a pair of
        #actions. If there is at least one pair of actions that are not mutually
        #exclusive, then the literals are not mutually exclusive either.
    ...
    def h_levelsum(self):
        ...
        #Find for every goal in the problem the level where they are first met,
        #and add all the levels in level_sum
